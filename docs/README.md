# Agentic Design Patterns Documentation

Comprehensive roadmap and implementation guide for LLM coding agents.

## Documentation Files

| File | Size | Purpose |
|------|------|---------|
| **00_ROADMAP_INDEX.md** | 11K | Master index, navigation, pattern selector |
| **01_FRAMEWORKS_COMPARISON.md** | 17K | Google ADK vs LangChain/OpenAI deep dive |
| **02_CORE_CONCEPTS.md** | 24K | Session state, events, tools, relationships |
| **03_PARALLELIZATION_PATTERNS.md** | 19K | ParallelAgent, SequentialAgent, Hybrid patterns |
| **04_TOOL_USE_PATTERNS.md** | 7K | Google Search, Code Execution, Vertex AI Search |
| **05_MULTIAGENT_COLLABORATION.md** | 9K | Coordinator, Loop, AgentTool patterns |
| **06_GOAL_SETTING_ITERATION.md** | 8K | LLM-as-judge, iterative refinement |
| **07_DECISION_FRAMEWORKS.md** | 10K | Flowcharts, matrices, trade-off analysis |
| **08_TROUBLESHOOTING.md** | 9K | Common errors and solutions |
| **09_QUICK_REFERENCE.md** | 12K | Imports, signatures, templates cheatsheet |

**Total**: ~125KB of comprehensive documentation

## Quick Start

1. **New to agentic patterns?** → Start with [00_ROADMAP_INDEX.md](00_ROADMAP_INDEX.md)
2. **Need to choose a framework?** → Read [01_FRAMEWORKS_COMPARISON.md](01_FRAMEWORKS_COMPARISON.md)
3. **Want to understand core concepts?** → See [02_CORE_CONCEPTS.md](02_CORE_CONCEPTS.md)
4. **Looking for specific pattern?** → Use the Quick Pattern Selector in [00_ROADMAP_INDEX.md](00_ROADMAP_INDEX.md)
5. **Need quick code snippet?** → Check [09_QUICK_REFERENCE.md](09_QUICK_REFERENCE.md)
6. **Having issues?** → See [08_TROUBLESHOOTING.md](08_TROUBLESHOOTING.md)

## Key Features

✅ **Complete Runnable Examples**: All code blocks include imports, setup, and execution  
✅ **Modular Organization**: Each file covers one topic area comprehensively  
✅ **Decision Frameworks**: Flowcharts, matrices, and comparative analyses  
✅ **Complementarity Analysis**: Which patterns work well together  
✅ **Orthogonality Analysis**: Independent dimensions/features across patterns  
✅ **LLM-Optimized**: Clear structure for agent consumption and reference

## Patterns Covered

### Google ADK Patterns
- **Parallelization**: ParallelAgent, SequentialAgent, Hybrid
- **Tool Use**: Google Search, Code Execution, Vertex AI Search
- **Multi-Agent Collaboration**: Coordinator, Loop, AgentTool

### LangChain + OpenAI Patterns
- **Goal Setting**: Iterative refinement with LLM-as-judge

## Documentation Philosophy

- **Complete Over Concise**: Full working examples, not fragments
- **Practical Over Theoretical**: Real-world decision criteria
- **Searchable Over Narrative**: Easy to find specific information
- **Reference-Friendly**: Designed for quick lookup

## Source Material

Based on comprehensive analysis of:
- 10 Jupyter notebooks from "Agentic Design Patterns" by Antonio Gulli
- 2 frameworks: Google ADK (Gemini) and LangChain (OpenAI)
- 8 distinct agentic patterns with complete implementations

## Navigation

Start at [00_ROADMAP_INDEX.md](00_ROADMAP_INDEX.md) for the complete navigation guide.
